...
public class BattleFieldFighting {
	...
	public void startAttack() {
			...
			public void run() {
				if (totalHitpointDefenceAfter == totalHitpointDefenceBefore) {
					...
					for (int j = 0; j < IsLandUI.house[houseID].getWaitingWaveAttack().size(); j++) {
						...
						}, attackBattleField.getReserve().toArmy().getSpeedTimeForWholeRoute());
					}
					// Check xem quân địch hay quân ta đã chết hết chưa, nếu chưa thì bắt đầu hiệp tiếp theo.
		...
		}, (15*60*1000)/IsLandUI.timeSpeed); // Thời gian mỗi hiệp 15p
